...
final class OmiLaunchReadinessTests: XCTestCase {
    ...
    func testWriterStartWaitsForLaunchReadinessAndIsIdempotent() async throws {
        ...
        manager.handleAudioData(.payload(Self.audioPacket(0)), peripheralID: UUID())
        manager.handleAudioData(.payload(Self.audioPacket(1)), peripheralID: UUID())
        ...
        manager.handleAudioData(.payload(Self.audioPacket(2)), peripheralID: UUID())
