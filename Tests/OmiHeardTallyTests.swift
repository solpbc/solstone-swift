...
nonisolated final class OmiHeardTallyTests: XCTestCase {
    private var tempDirectory: URL!

    override func setUp() {
        super.setUp()
        self.tempDirectory = FileManager.default.temporaryDirectory
            .appendingPathComponent("OmiHeardTallyTests-\(UUID().uuidString)", isDirectory: true)
    ...
    func testRecordIsIdempotentForSameIdentityOnSameDay() {
        let tally = OmiHeardTally(fileURL: self.fileURL())
    ...
    func testMissingAndCorruptFilesFailOpenToEmpty() throws {
        let missing = OmiHeardTally(fileURL: self.fileURL("missing.json"))
        ...
        let corrupt = OmiHeardTally(fileURL: corruptURL)
    ...
    func testPrunesToSevenMostRecentKeysOnLoadAndRecord() throws {
        ...
        try self.writePayload(
            Dictionary(uniqueKeysWithValues: (1...9).map { index in
                ...
                return (day, OmiHeardDayTally(totalSeconds: 60, seenIdentities: [day]))
            }),
            to: fileURL
        )
    ...
    private func fileURL(_ name: String = "omi-heard-tally.json") -> URL {
        self.tempDirectory.appendingPathComponent(name, isDirectory: false)
    }

    private func writePayload(_ payload: OmiHeardTallyPayload, to fileURL: URL) throws {
