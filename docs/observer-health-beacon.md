...
The observer health beacon is a diagnostics-only POST from each observer audio upload source: mobile, omi, and watch. It reports upload queue health to the paired journal at `/app/observer/health` using the existing observer bearer handle. It does not include captured audio, transcript content, captured file paths, or location payloads.
